...
void main()
{
	...
	if (mode < 0.0f)
	{
		vColor = (NdL * vec4(matColor,1) / 1.0f + spec) / 1.0f;
	}
	else if (mode < 10.0f)
	{
		vColor = (NdL * texture2D(tex, TexCoord) / 1.0f + spec) / 1.0f;
	}
	else if (mode == 10.0f)
	{
		vColor = vec4(200.0f, 200.0f, 200.0f, 1.0f);
	}
	...
	aa = pow(20.0f, compFactor) / aa - pow(20.0f, compFactor);
	float ldrY = Y / (Y + aa);
	vec3 ldr;
